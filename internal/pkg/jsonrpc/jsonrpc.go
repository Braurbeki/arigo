// Package jsonrpc implements a JSON-RPC ClientCodec and ServerCodec for rpc2.
// This is a local copy with relaxed error parsing to tolerate JSON-RPC 2.0 error objects.
package jsonrpc
...
func (c *jsonCodec) ReadHeader(req *rpc2.Request, resp *rpc2.Response) error {
	...
	if c.msg.Method != "" {
		...
		if c.clientResponse.Error != nil || c.clientResponse.Result == nil {
			resp.Error = formatError(c.clientResponse.Error)
			if resp.Error == "" {
				resp.Error = "unspecified error"
			}
		}
	}
	return nil
...
func formatError(errVal interface{}) string {
	...
	case map[string]interface{}:
		...
		return fmt.Sprintf("rpc error: %v", e)
	default:
		return fmt.Sprintf("invalid error %v", e)
	}
}
